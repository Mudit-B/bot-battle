...
class BotState():
    def __init__(self):
        ...
        self.wait_flag_if_needed = False
    ...
    def update_starting_target(self, game):
        """Update the target continent based on the current game state."""
        continents = game.state.map.get_continents()
        my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    ...
    #         # If there are no more remaining vertices, return the current path
    #         if not remaining:
    #             return path
            
    #         longest_path = path
    #         for neighbor in self.map_connections.get(current, []):
    #             if neighbor in remaining:
    #                 new_path = dfs(neighbor, path.copy(), remaining.copy())
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    our_choke_points = bot_state.choke_points & set(my_territories)
    threats = []
    for terr in our_choke_points:
        threat = max([game.state.territories[t].troops for t in set(game.state.map.get_adjacent_to(terr)) - set(my_territories)], default=0)
        our_max = max([game.state.territories[t].troops for t in set(game.state.map.get_adjacent_to(terr)) & set(my_territories)], default=0)
        our_max = max(our_max, game.state.territories[terr].troops)
        diff = threat - our_max
        if diff <= 0:
            continue
        else:
            threats.append((diff, terr))
    
    threats.sort(reverse=True)
    # threats = threats[:2] # just pick 2 always
    for diff, terr in threats:
        if total_troops > diff:
            total_troops -= diff
            distributions[terr] += diff
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    if bot_state.wait_flag_if_needed:
    ...
    def is_positive_move(attacker, target):
        ...
        if 1.2 * sum_continent_enemy < my_score:
            bot_state.wait_turn = False
            return True
        elif sum_continent_enemy < my_score:
            bot_state.wait_turn = True
            return True
        
        return False
    
    def checks_put_in_vulnerable(attacker, target):
        attacker_troops = game.state.territories[attacker].troops
        target_troops = game.state.territories[target].troops
        ...
        if attacker_troops > get_threat(attacker) + get_threat(target):
            return True
        return False
    ...
    def attack_highest_probability(territories: list[int]):
        ...
        for target in territories:
            ...
            for attacker in potential_attackers:
                ...
                elif (attacker_troops - target_troops >= 2) and probability > actual_best and checks_put_in_vulnerable(attacker, target):
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    bot_state.wait_flag_if_needed = bot_state.wait_turn
    ...
    def get_threat(territory):
        adjacent_enemies = set(game.state.map.get_adjacent_to(territory)) - set(mine)
        return max([game.state.territories[t].troops for t in adjacent_enemies], default=0)
    ...
    if (bot_state.path == []):
        threat_level_of_attacker = get_threat(attacker)
        if threat_level_of_attacker > 0:
            min_troops = min(3, game.state.territories[move_attack.attacking_territory].troops - 1)
            move_troops = max(min_troops, game.state.territories[move_attack.attacking_territory].troops - threat_level_of_attacker)
            return game.move_troops_after_attack(query, move_troops)
...
def handle_fortify(game: Game, bot_state: BotState, query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    bot_state.wait_flag_if_needed = False
    bot_state.wait_turn = False
    """
