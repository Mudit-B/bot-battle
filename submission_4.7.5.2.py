...
def handle_redeem_cards(game: Game, bot_state: BotState, query: QueryRedeemCards) -> MoveRedeemCards:
    """Redeem cards as soon as they are available, while following the rules for redemption after eliminating a player."""
    card_sets: list[Tuple[CardModel, CardModel, CardModel]] = []
    cards_remaining = game.state.me.cards.copy()

    while True:
        # If redeeming after killing a player and we have less than 5 cards, stop
        if query.cause == "player_eliminated" and len(cards_remaining) < 5:
            break
    ...
    # If we have 5 or more cards at the start of our turn, we must redeem at least one set
    if query.cause == "turn_started" and len(game.state.me.cards) >= 5 and not card_sets:
        # This should never happen due to the pigeonhole principle, but just in case
        raise ValueError("Unable to form a card set with 5 or more cards.")
