...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    def eval_terr(territory):
        ...
        value += 2 * max(set(game.state.map.get_adjacent_to(territory)) - set(my_territories), key=lambda t: game.state.territories[t].troops, default=0)
        return value
...
def handle_fortify(game: Game, bot_state: BotState, query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    # Define choke points (adjust as needed based on the map layout)
    choke_points = bot_state.choke_points
    
    def is_choke_point(territory):
        return territory in choke_points

    def get_threat(territory):
        return sum(game.state.territories[t].troops for t in set(game.state.map.get_adjacent_to(territory)) - set(my_territories))

    # Calculate fortification values and find the best move
    best_move = None
    ...
    for threat_level, target_territory in candidate_threat:
        for territory in most_troops_territory:
            ...
            troops = math.floor((source_troops - 1) * fraction_to_send)
            troops = max(troops, 1)
