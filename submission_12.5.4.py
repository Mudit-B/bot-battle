...
class BotState():
    def __init__(self):
        ...
        self.north_america = set(range(0, 9))
        self.europe = set(range(9, 16))
        self.asia = set(range(16, 28))
        self.choke_points = set([29, 30, 2, 0, 4, 10, 13, 14, 15, 36, 34, 33, 22, 16, 26, 21, 24, 40])

        self.na_queue = [0, 4, 2]
        self.sa_queue = [30, 29, 28]
        self.south_africa_queue = [36, 34, 33]
        self.europe_queue = [10, 13, 14, 15]
        self.claimed_continents = []
    ...
    def update_target_continent(self, game: Game):
        ...
        for continent in continent_control:
            if continent_control[continent][0] == 1:
                ...
                self.claimed_continents.append(continent)
...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    def check_someone_else_claimed(game, continent):
        for t in game.state.map.get_continents()[continent]:
            if game.state.territories[t].occupier not in [None, game.state.me.player_id]:
                return True
        return False

    # Take Australia
    if not check_someone_else_claimed(game, 5):
        left_australia = set(unclaimed_territories) & set(bot_state.australia)
        if left_australia:
            return game.move_claim_territory(query, random.choice(list(left_australia)))

    # Take South Africa
    if (not check_someone_else_claimed(game, 4)) or (len(bot_state.south_africa_queue) <= 2):
        africa_territories = set(game.state.map.get_continents()[4]) & set(unclaimed_territories)
        if africa_territories:
            if bot_state.south_africa_queue:
                choice = bot_state.south_africa_queue.pop(0)
                if choice in africa_territories:
                    return game.move_claim_territory(query, choice)
            return game.move_claim_territory(query, random.choice(list(africa_territories)))
    
    # Take South America
    if (not check_someone_else_claimed(game, 3)) or (len(bot_state.sa_queue) <= 2):
        sa_territories = set(game.state.map.get_continents()[3]) & set(unclaimed_territories)
        if sa_territories:
            if bot_state.sa_queue:
                choice = bot_state.sa_queue.pop(0)
                if choice in sa_territories:
                    return game.move_claim_territory(query, choice)
            return game.move_claim_territory(query, random.choice(list(sa_territories)))

    # Try claiming adjacent friendlies
    ...
    if available:
        return game.move_claim_territory(query, max(available, key=lambda x: len(set(my_territories) & set(game.state.map.get_adjacent_to(x)))))

    # any territory
    return game.move_claim_territory(query, random.choice(list(unclaimed_territories)))
...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    def is_choke_point(territory):
        return territory in bot_state.choke_points
    ...
    def get_threat(territory):
        adjacent_enemies = set(game.state.map.get_adjacent_to(territory)) - set(my_territories)
        return max([game.state.territories[t].troops for t in adjacent_enemies], default=0)

    def is_potential_attacker(territory):
        adjacent_enemies = set(game.state.map.get_adjacent_to(territory)) - set(my_territories)
        territory_continent = get_continent_of_territory(territory)
        can_attack_target = any(get_continent_of_territory(enemy) == target_continent for enemy in adjacent_enemies)
        return (territory_continent == target_continent or can_attack_target) and \
               any(game.state.territories[territory].troops > game.state.territories[enemy].troops for enemy in adjacent_enemies)

    # Sort territories by priority
    priority_territories = sorted(
        border_territories,
        key=lambda t: (is_choke_point(t), is_potential_attacker(t), get_threat(t)),
        reverse=True
    )

    # Choose the highest priority territory
    for territory in priority_territories:
        threat = get_threat(territory)
        current_troops = game.state.territories[territory].troops

        if is_choke_point(territory):
            if current_troops <= threat:
                return game.move_place_initial_troop(query, territory)
        elif is_potential_attacker(territory):
            if current_troops <= threat + 4:
                return game.move_place_initial_troop(query, territory)

    return game.move_place_initial_troop(query, max(priority_territories, key=get_threat))
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    if game.state.me.must_place_territory_bonus:
        bonus_territory = game.state.me.must_place_territory_bonus[0]
        distributions[bonus_territory] += 2
        total_troops -= 2
    ...
    claimed_continent_choke_points = [
        t for t in border_territories
        if is_choke_point(t) and get_continent_of_territory(t) in bot_state.claimed_continents
    ]

    for territory in claimed_continent_choke_points:
        if total_troops <= 0:
            break

        threat = get_threat(territory)
        current_troops = game.state.territories[territory].troops
        troops_needed = max(0, threat - current_troops)

        troop_share = min(total_troops, troops_needed)
        distributions[territory] += troop_share
        total_troops -= troop_share

    # Sort remaining territories by priority
    priority_territories = sorted(
        [t for t in border_territories if t not in claimed_continent_choke_points],
        key=lambda t: (is_choke_point(t), is_potential_attacker(t), get_threat(t)),
        reverse=True
    )

    # Distribute troops to remaining territories
    for territory in priority_territories:
        if total_troops <= 0:
            break
        ...
        if is_choke_point(territory):
            troops_needed = max(0, threat - current_troops + 1)
            troop_share = min(total_troops, troops_needed)
        elif is_potential_attacker(territory):
            troops_needed = max(0, threat - current_troops + 5)
            troop_share = min(total_troops, troops_needed)
        else:
            troop_share = min(total_troops, max(1, total_troops // 20))

        distributions[territory] += troop_share
        total_troops -= troop_share

    potential_attackers = [t for t in priority_territories if is_potential_attacker(t)]
    while total_troops > 0 and potential_attackers:
        for territory in potential_attackers:
            if total_troops <= 0:
                break
    ...
    if total_troops > 0:
        territories_to_reinforce = list(my_territories)
        while total_troops > 0:
            for territory in territories_to_reinforce:
                if total_troops <= 0:
                    break
                distributions[territory] += 1
                total_troops -= 1
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    def is_positive_move(attacker, target):
        ...
        # Check if the attacking territory becomes vulnerable
        troops_left_after_attack = attacker_troops - min(3, attacker_troops - 1)
        max_adjacent_enemy_troops = max([game.state.territories[t].troops for t in adj_attacker], default=0)
        
        # fewer troops than the strongest adjacent enemy
        if troops_left_after_attack <= max_adjacent_enemy_troops:
            return False
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    def get_threat(territory):
        max_enemy = 0
        for t in set(game.state.map.get_adjacent_to(territory)) - set(mine):
            max_enemy = max(max_enemy, game.state.territories[t].troops)
        return max_enemy
    ...
    if (curr_adj_to_attacker <= (enemy_adj_to_target)) or (bot_state.path != []):
        ...
        # If 2 players and not figured path yet.
        if bot_state.get_num_alive(game) == 2 and (bot_state.path == []):
        ...
        move_troops = None
        if attacker in bot_state.choke_points:
            threat_choke_point = get_threat(attacker)
            if threat_choke_point == 0:
                return game.move_troops_after_attack(query, game.state.territories[move_attack.attacking_territory].troops - 1)
            
            leave_behind_troops = 1 + threat_choke_point

            min_troops = min(3, game.state.territories[move_attack.attacking_territory].troops - 1)
            move_troops = max(min_troops, game.state.territories[move_attack.attacking_territory].troops - leave_behind_troops)            
            return game.move_troops_after_attack(query, move_troops)        
        return game.move_troops_after_attack(query, game.state.territories[move_attack.attacking_territory].troops - 1)        
    # path 
    else:
