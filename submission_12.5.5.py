...
class BotState():
    def __init__(self):
        ...
        self.choke_points = set([29, 30, 2, 0, 4, 10, 13, 14, 15, 36, 34, 33, 22, 21, 24, 40])
        self.path = []
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    def is_positive_move(attacker, target):
        ...
        if get_continent_of_territory(attacker) != get_continent_of_territory(target):
            ...
            # check if it leaves this continent vulnerable
            attacker_continent = get_continent_of_territory(attacker)
            enemies = set(game.state.map.get_continents()[attacker_continent]) - set(my_territories)
            mine = (set(game.state.map.get_continents()[attacker_continent]) & set(my_territories)) - set([attacker])
            sum_curr_enemies = sum([game.state.territories[i].troops for i in enemies])
            sum_mine = sum([game.state.territories[i].troops for i in mine])
            if sum_mine < sum_curr_enemies:
                return False
    ...
    if move:
        if isContinentCapture:
            ...
            if not bot_state.conquered_once:
                return move
        return move
