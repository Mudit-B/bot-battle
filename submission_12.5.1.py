...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    # Calculate threat level for each territory
    def calculate_threat(territory):
        adjacent_enemy_territories = set(game.state.map.get_adjacent_to(territory)) - set(my_territories)
        return sum(game.state.territories[adj_terr].troops for adj_terr in adjacent_enemy_territories)

    # Find all choke points and evaluate their threat and value
    choke_points = []
    for territory in my_territories:
        if len(set(game.state.map.get_adjacent_to(territory)) & set(border_territories)) > 0:
            choke_points.append(territory)

    # Prioritize troop placement in threatened choke points
    for choke_point in choke_points:
        threat_level = calculate_threat(choke_point)
        value = eval_terr(choke_point)
        if threat_level > value:
            distributions[choke_point] += 1
            total_troops -= 1
            if total_troops <= 0:
                break

    # Find the best border territory to distribute remaining troops
    best_border_territory = find_best_territory(border_territories)
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    if (curr_adj_to_attacker <= (enemy_adj_to_target)) or ((bot_state.path != []) and (not bot_state.mark_only_move_three)):
