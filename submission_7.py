...
class MiniMax:
    ...
    def apply_move(self, move):
        ...
        print("Attack from: ", from_territory,"To:", to_territory, flush=True)
    ...
    def heuristic(self) -> float:
        evaluation = 0
        my_territories = self.state.get_territories_owned_by(self.state.me.player_id)
        border_territories = self.state.get_all_border_territories(my_territories)
        enemy_territories = set(self.state.get_all_adjacent_territories(border_territories)) - set(my_territories)

        border_security_threat = sum([self.state.territories[t].troops for t in enemy_territories])

        for territory in border_territories:
            border_security_ratio = self.state.territories[territory].troops / border_security_threat
            evaluation += (1 / border_security_ratio)

        total_territories = len(self.state.territories)
        num_enemy_territories = len(enemy_territories)
        evaluation += num_enemy_territories / total_territories

        # # Add continent control evaluation
        # continents = self.game.state.map.get_continents()
        # my_territories = set(self.my_territories.keys())
        # for continent, territories in continents.items():
        #     control_ratio = len(set(territories) & my_territories) / len(territories)
        #     evaluation += control_ratio * self.game.state.map.get_continent_bonus(continent)

        print("Value of eval", evaluation, flush=True)
        return evaluation
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    best_move = minimax.get_best_move(3)
