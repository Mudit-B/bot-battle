...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    left_na = set(unclaimed_territories) & set(bot_state.australia)

    # or we have 3 in NA
    if (left_na != bot_state.north_america - my_territories) and len((bot_state.north_america & set(my_territories))) < 3:
        left_na = set()
    ...
    if len(available) != 0:
        if left_na:
            selected_territory = unclaimed_territories & bot_state.na_queue.pop(0)
        elif left_south_america:
            selected_territory = unclaimed_territories & bot_state.sa_queue.pop(0)
