...
def check_someone_else_claimed(game:Game, continent):
...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    # # Take North America
    # if (check_someone_else_claimed(game, 0) == False) or (len(bot_state.na_queue) <= 1):
    #     na_territories = set(game.state.map.get_continents()[0]) & set(unclaimed_territories)
    #     if na_territories:
    #         if bot_state.na_queue:
    #             choice = bot_state.na_queue.pop(0)
    #             if choice in na_territories:
    #                 return game.move_claim_territory(query, choice)
    #         return game.move_claim_territory(query, random.choice(list(na_territories)))

    # Take Australia
    if check_someone_else_claimed(game, 5) == False:
    ...
    # Take South Africa
    if (check_someone_else_claimed(game, 4) == False) or (len(bot_state.south_africa_queue) <= 2):
    ...
    # Take South America
    if (check_someone_else_claimed(game, 3) == False) or (len(bot_state.sa_queue) <= 2):
    ...
    # # Take Europe
    # if (check_someone_else_claimed(game, 1) == False) or (len(bot_state.europe_queue) <= 2):
    #     europe_territories = set(game.state.map.get_continents()[1]) & set(unclaimed_territories)
    #     if europe_territories:
    #         if bot_state.europe_queue:
    #             choice = bot_state.europe_queue.pop(0)
    #             if choice in europe_territories:
    #                 return game.move_claim_territory(query, choice)
    #         return game.move_claim_territory(query, random.choice(list(europe_territories)))
    ...
    def eval_terr(territory):
        ...
        for k, territories in continents.items():
            if territory in territories:
                if k == target_continent or k in bot_state.claimed_continents:
                    value += 50
                break
        
        adj_friendlies = len(set(game.state.map.get_adjacent_to(territory)) & set(my_territories))
        value += adj_friendlies
        
        current_troops = game.state.territories[territory].troops
        value -= (current_troops)
        return value

    sorted_territories = sorted(border_territories, key=eval_terr, reverse=True)
    return game.move_place_initial_troop(query, sorted_territories[0])
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    def get_threat(territory):
        max_enemy = 0
        for t in set(game.state.map.get_adjacent_to(territory)) - set(my_territories):
            max_enemy = max(max_enemy, game.state.territories[t].troops)
        if max_enemy < game.state.territories[territory].troops:
            return 0
        return max_enemy - game.state.territories[territory].troops
    ...
    def eval_terr(territory):
        ...
        for k, territories in continents.items():
            if territory in territories:
                continent = k
                break
        if (continent == target_continent) or ((continent in bot_state.claimed_continents) and (territory in bot_state.choke_points)):
            if continent == target_continent:
                value += 100
            else:
                value += 20 * get_threat(territory)

        # 1% this too hit and trial this.
        value += 0.5 * len(set(game.state.map.get_adjacent_to(territory)) & set(my_territories))
        # value += max((set(game.state.map.get_adjacent_to(territory)) - set(my_territories)), key=lambda x: game.state.territories[x].troops, default=0)
        return value
