...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    for territory in border_territories:
        ...
        threat = sum(game.state.territories[enemy].troops for enemy in adjacent_enemies)
        threat_levels[territory] = threat

    # the territory with the highest threat
    best_territory = max(threat_levels, key=threat_levels.get)

    return game.move_place_initial_troop(query, best_territory)
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    # get threat levels for border territories
    threat_levels = {}
    for territory in border_territories:
        adjacent_enemies = set(game.state.map.get_adjacent_to(territory)) - set(my_territories)
        threat = sum(game.state.territories[enemy].troops for enemy in adjacent_enemies)
    ...
    # threat levels into decimals  
    total_threat = sum(threat_levels.values())
    if total_threat > 0:
        for territory in threat_levels:
            threat_levels[territory] /= total_threat
    else:
        # no threat, distribute equally (should never happen except the case when the game is over)
        equal_share = 1 / len(border_territories)
        threat_levels = {t: equal_share for t in border_territories}

    # give troops based on threat levels
    if bot_state.get_num_alive(game) == 2:
        # End game logic (2 players left)
    ...
    for territory, threat in threat_levels.items():
        troops_to_add = int(total_troops * threat)
        distributions[territory] += troops_to_add
        total_troops -= troops_to_add

    # any remaining troops goes to territory with the biggest threat
    if total_troops > 0:
        # .get can be used as a key just like .len
        highest_threat_territory = max(threat_levels, key=threat_levels.get)
        distributions[highest_threat_territory] += total_troops
