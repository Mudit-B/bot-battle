        ...
        for k, v in game.state.map.get_continents().items():
            if target in v:
                ...
                for choke_point in choke_pts:
                    ...
                    print(max(other_enemies, key=lambda t: game.state.territories[t].troops, default=0), flush= True)
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    # print(len(game.state.recording), bot_state.path, 'HERE submission_12.pysubmission_12.pyin handle troops after attack', flush=True)
