...
class BotState():
    ...
    def update_target_continent(self, game: Game): self.update_target_continents(game)
    def update_target_continents(self, game: Game):
        ...
        # if continent_control[self.target_continent][0] * len(continents[self.target_continent]) == 1:
        #     # this means we dont really have any prefrence of target continent
        #     self.target_continents = []
        #     for continent in continent_control:
        #         if continent_control[continent][0] != -1:
        #             self.target_continents.append(continent)        
        # else:
        #     self.target_continents = []
        #     for continent in continent_control:
        #         if continent_control[continent][0] == continent_control[self.target_continent][0]:
        #             self.target_continents.append(continent)
        self.target_continents = [self.target_continent]
def main():
...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    my_territories = set(my_territories)
    ...
    def eval_terr(territory):
        ...
        for k, territories in continents.items():
            if territory in territories:
                continent = k
                break
        enemies = set(game.state.map.get_adjacent_to(territory)) - set(my_territories)
        
        for enemy in enemies:
            for k, terrs in continents.items():
                if enemy in terrs and k == target_continent:
                    value += 100
                    break
        # 1% this too hit and trial this.
        value += 0.5 * len(set(game.state.map.get_adjacent_to(territory)) & set(my_territories))
        value += game.state.territories[territory].troops
        return value
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    bot_state.mark_only_move_three = False
    ...
    bot_state.update_target_continents(game)
    target_continents = bot_state.target_continents
    ...
    def is_positive_move(attacker, target):
        ...
        attacker_continents = bot_state.target_continents
        ...
        if 2 * sum_continent_enemy < attacker_troops:
            return True
        return False
    ...
    def attack_highest_probability(territories: list[int]) -> Optional[MoveAttack]:
        ...
        for target in territories:
            ...
            for attacker in potential_attackers:
                ...
                elif (attacker_troops - target_troops >= 3):
                    other_moves.append((attacker, target, probability))

        # This is the case where we might get sorta stuck
        if not best_move:
            if not other_moves: 
                return best_move
            best_probability = 0
            for attacker, target, probability in other_moves:
                continents = game.state.map.get_continents()
                lst_territories = []
                for continent in bot_state.target_continents:
                    lst_territories.extend(continents[continent])
                if  probability > best_probability and target in lst_territories:
                    best_probability = probability
                    best_move = game.move_attack(query, attacker, target, min(3, game.state.territories[attacker].troops - 1))
                    print(len(game.state.recording), best_move, flush=True)
    ...
    def find_best_target_continent(territories: list[int]) -> list[int]:
        ...
        for territory in territories:
            for continent, continent_territories in continents.items():
                if continent in target_continents and territory in continent_territories:
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    if bordering:
        ...
        if bot_state.mark_only_move_three:
            bot_state.mark_only_move_three = False
