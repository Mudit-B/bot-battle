...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    best_border_territory = None
    # if bot_state.get_num_alive(game) == 2:
    #     # doom stack.
    #     best_border_territory = max(border_territories, key = lambda x: game.state.territories[x].troops)
    # else:
    #     # Find the best border territory to distribute troops
    best_border_territory = find_best_territory(border_territories)
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    if bot_state.flag_leave_behind_troops == True:
        print("here", flush=True)
        # Leave troops behind based on the final enemy path
        path = [i for i in bot_state.path if i not in mine]

        # sum it
        path_sum = sum([game.state.territories[i].troops for i in path])

        # enemy total troops
        total_troops = sum([game.state.territories[i].troops for i in game.state.get_territories_owned_by(bot_state.enemy)])

        # fraction to path (SHOULDNT BE 0 right?)
        fraction = path_sum / total_troops

        total_troops = game.state.territories[move_attack.attacking_territory].troops
        troops_to_allocate = math.floor(fraction * total_troops)
        min_troops = min(3, game.state.territories[move_attack.attacking_territory].troops - 1)
        troops_to_allocate = max(troops_to_allocate, min_troops)
        troops_to_allocate = min(troops_to_allocate, total_troops - 1)

        bot_state.flag_leave_behind_troops = False

        return game.move_troops_after_attack(query, troops_to_allocate)
    # if bot_state.flag_continent_capture_leave == True:
    #     # Leave troops behind based on the final enemy path
    #     path = [i for i in bot_state.path if i not in mine]
    ...
    #     # enemy total troops
    #     if path != []:
    #         total_troops = sum([game.state.territories[i].troops for i in get_territories_of_continent_of_territory(path[0])])
    ...
    if (curr_adj_to_attacker <= (enemy_adj_to_target)) or (bot_state.path != []):
        ...
        if bot_state.get_num_alive(game) == 2 and ((bot_state.path == []) and (bot_state.killing_final == False)):
    ...
    # path 
        ...
        # # threat levels for border territories
        # attacker_threat = max(set(game.state.map.get_adjacent_to(attacker)) - set(my_territories), key=lambda t: game.state.territories[t].troops, default=0)
        # target_threat = max(set(game.state.map.get_adjacent_to(target)) - set(my_territories), key=lambda t: game.state.territories[t].troops, default=0)

        # total_threat = attacker_threat + target_threat
        # if total_threat == 0:
        #     # If there is no enemy then move only 3
        #     move_troops = min(3, game.state.territories[move_attack.attacking_territory].troops - 1)
        #     return game.move_troops_after_attack(query, move_troops)
        
        # fraction = target_threat / total_threat
        # if fraction == 0:
        #     # If there is no enemy then move only 3
        #     move_troops = min(3, game.state.territories[move_attack.attacking_territory].troops - 1)
        #     return game.move_troops_after_attack(query, move_troops)
...
def handle_fortify(game: Game, bot_state: BotState, query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    for territory in my_territories:
        if territory in bot_state.choke_points:
    ...
    for threat_level, target_territory in candidate_threat:
        for territory in most_troops_territory:
            ...
            if threat_level < threat:
                ...
                zero_change.append((territory, target_territory, source_troops - 1))
                continue
    ...
    zero_change.sort(key=lambda x: x[2], reverse=True)
    if zero_change:
        best_move = zero_change[0]
        return game.move_fortify(query, best_move[0], best_move[1], best_move[2])
